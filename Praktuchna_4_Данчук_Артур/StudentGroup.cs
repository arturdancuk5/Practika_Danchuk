...
public class StudentGroup
{
    ...
    public Student this[string recordBookNumber]
    {
        get
        {
            ...
            var student = _students.FirstOrDefault(item =>
                item.RecordBookNumber == recordBookNumber.Trim());

            if (student == null)
            {
                throw new KeyNotFoundException("Студента з таким номером залікової не знайдено.");
            }
    ...
    private string GenerateUniqueRecordBookNumber(string baseNumber)
    {
        ...
        for (var attempt = 0; attempt < 1000; attempt++)
        {
            ...
            if (!_students.Any(student => student.RecordBookNumber == candidate))
            {
                return candidate;
            }
        }
