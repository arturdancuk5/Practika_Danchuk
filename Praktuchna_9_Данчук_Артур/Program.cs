...
internal class Program
{
            ...
            try
            {
                switch (choice)
                {
                    ...
                    case "19":
                        ...
                        break;
                    case "20":
                        await GenerateHeavyReportMenuAsync();
                        break;
                    case "21":
                        AnalyzeStudentClass();
                        break;
                    case "22":
                        ReadDeveloperMetadata();
                        break;
                    case "0":
                        running = false;
                        break;
    ...
    private static void ShowMenu()
    {
        ...
        Console.WriteLine("20. Згенерувати складний аналітичний звіт");
        Console.WriteLine("21. Аналіз класу Student через рефлексію");
        Console.WriteLine("22. Прочитати метадані розробника");
    ...
    private static async Task SaveOrLoadAsync()
    {
        ...
        if (choice == "1")
        {
            await _group.SaveToFileAsync(DataFileName);
            Console.WriteLine($"Дані збережено у файл {DataFileName}.");
        }
        else if (choice == "2")
        {
            _group = await StudentGroup.LoadFromFileAsync(DataFileName);
    ...
    private static async Task GenerateHeavyReportMenuAsync()
    {
        Console.WriteLine("Генерація звіту почалась...");
        var report = await _group.GenerateHeavyReportAsync();
        Console.WriteLine(report);
    }

    private static void AnalyzeStudentClass()
    {
        ReflectionAnalyzer.AnalyzeClassMetadata(typeof(Student));
    }

    private static void ReadDeveloperMetadata()
    {
        ReflectionAnalyzer.GetDeveloperInfo(typeof(Student));
    }
